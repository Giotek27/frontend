<template>
<div>
<h1>Tabla de datos de ventas registradas</h1>
    <div>
        <nav class="navbar navbar-expand-lg navbar-dark bg-success">
            ...
            <b-navbar-nav class="ml-auto">
                 <b-nav-item>
                    <router-link :to="{name:'crearventas'}" class="btn btn-outline-light" >
        ...
        <tbody>
            <tr id ="tabla" v-for="usuario in usuarios" :key="usuario.id">
                ...
                <td>
                    ...
                    <router-link :to="{name: 'editarventas', 
...
<script>
    ...
    export default {
        data() {
            ...
            created(){
                axios.get("https://backend54-7.herokuapp.com/consultarventas").then((res)=>this.usuarios=res.data);
            },
            methods:{
                deleteusuario(id){
                let indexofArrayItem=this.usuarios.findIndex((i)=>i._id === id);
                axios.delete("https://backend54-7.herokuapp.com/borrarventas/"+id).then(()=>console.log("El registro se ha eliminado"));
                this.usuarios.splice(indexofArrayItem,1);
                alert("El registro se ha eliminado");
            },
            consultarVendedorN(){
                const API="https://backend54-7.herokuapp.com/buscaventas/"+this.buscarusuario;
                console.log(API);
